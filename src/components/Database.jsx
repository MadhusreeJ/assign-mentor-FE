...
const Database = () => {
    ...
    let getData = async () => {
        try {
          const students = await axios.get("https://assign-mentor-be-9cez.onrender.com/getallstudents");
      ...
      let getFilteredData = async (event,selectedMentor) => {
        ...
        if(selectedMentor.mentor === 'all' ){
          ...
          try {
            const students = await axios.get(`https://assign-mentor-be-9cez.onrender.com/studentsmentor/${selectedMentor._id}`);
            setStudents(students.data);
          } catch (error) {
            console.log(error);
          }
